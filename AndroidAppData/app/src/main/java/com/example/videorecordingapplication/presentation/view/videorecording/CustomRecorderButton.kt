...
import com.example.videorecordingapplication.presentation.view.videorecording.RecorderStateManager.RecorderState.*
...
class CustomRecorderButton @JvmOverloads constructor(
    ...
    fun setExperimentType(experiment: String) {
        this.experimentType = experiment
    }

    fun initializeInnerCircleSingleTapAnimation() {
    ...
    fun initializeInnerCircleLongPressAnimation() {
    ...
    fun initializeOuterCircleBorderAnimation() {
    ...
    internal fun handleRecordButtonClicked() {
    ...
    private fun onClickEnd() {
        ...
        } else if (actionListener?.getCurrentRecorderState() == RECORDING
                || actionListener?.getCurrentRecorderState() == PAUSED
                || actionListener?.getCurrentRecorderState() == RESUMED) {
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        if (actionListener?.getCurrentRecorderState() == INIT) {
            canvas.drawArc(
        ...
        if (actionListener?.getCurrentRecorderState() != INIT) {
            if (mCurrentRecordStatus == RECORD_STATUS_START) {
                canvas.drawArc(
                        outerCircleBorderRect,
                        -90f,
            ...
            } else if (mCurrentRecordStatus == RECORD_STATUS_PAUSE) {
            ...
            } else if (mCurrentRecordStatus == RECORD_STATUS_RESUME) {
                startAngle = pauseAngleList[0]
                sweepAngle = 0f
                var last = 0
                for ((i, pausedTime) in pauseTimeList.withIndex()) {
                ...
                outerCircleBorderPaint.color = Color.WHITE
                startAngle += sweepAngle
                sweepAngle = calculateAngle(resumeTimeList[last + 1], System.currentTimeMillis())
                canvas.drawArc(
                        outerCircleBorderRect,
                        startAngle,
                        sweepAngle,
            ...
            } else if (mCurrentRecordStatus == RECORD_STATUS_STOP) {
                startAngle = pauseAngleList[0]
                sweepAngle = 0f
                var last = 0
                for ((i, pausedTime) in pauseTimeList.withIndex()) {
    ...
    fun getRecordingDuration() = videoDurationInMillis
}
